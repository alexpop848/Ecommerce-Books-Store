...
function sendAllDownloadLinks(email, downloadableItems) {
    ...
    return sendEmail({
        ...
        htmlContent: downloadableItems.map(({ item, code }) => {
            return `<a href="${process.env.SERVER_URL}/downloads/${code}">Download ${item.name}</a>`
        }).join('<br>'),
        textContent: downloadableItems.map(({ item, code }) => {
            return `Download ${item.name} ${process.env.SERVER_URL}/downloads/${code}`
        }).join('<\n>'),
    })
}
